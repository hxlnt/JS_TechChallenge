...
Blockchain.prototype.verify = function() {
  return fetch('https://chessblockchain-fa.azurewebsites.net/api/verify', { headers: { 'content-type': 'application/json'}, method: 'POST', body: JSON.stringify({ chain: this.serialize() })})
  .then(function(res) {
      return res.json();
  }).then(function(json) {
      console.log(json);
